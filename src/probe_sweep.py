...
def oof_acc(X, y, K, seed=SEED, **kw):
...
def mean_oof(X, y, K, repeats, **kw):
    # average over several fold splits to shrink the fold noise
    accs = [oof_acc(X, y, K, seed=SEED + r, **kw) for r in range(repeats)]
...
def main():
    ...
    p.add_argument("--repeats", type=int, default=1, help="avg OOF over N fold splits to cut noise")
    ...
    for penalty, solver in combos:
        for cw in (None, "balanced"):
            for C in Cs:
                m, s = mean_oof(X, y, K, args.repeats, C=C, penalty=penalty,
                                solver=solver, class_weight=cw)
                cfg = dict(C=C, penalty=penalty, solver=solver, class_weight=cw)
                results.append((m, s, cfg))
                print(f"{m:.4f} +/-{s:.4f}  C={C:<7g} {penalty:<3} {solver:<10} cw={cw}")

    if args.saga:
        for l1r in (0.1, 0.5, 0.9):
            for cw in (None, "balanced"):
                for C in Cs:
                    m, s = mean_oof(X, y, K, args.repeats, C=C, penalty="elasticnet",
                                    solver="saga", l1_ratio=l1r, class_weight=cw)
                    cfg = dict(C=C, penalty="elasticnet", solver="saga", l1_ratio=l1r, class_weight=cw)
                    results.append((m, s, cfg))
                    print(f"{m:.4f} +/-{s:.4f}  C={C:<7g} elasticnet saga l1r={l1r} cw={cw}")

    results.sort(key=lambda r: -r[0])
    print(f"\ntop {args.top}:")
    for m, s, cfg in results[:args.top]:
        print(f"  {m:.4f} +/-{s:.4f}  {cfg}")
    best_m, best_s, best_cfg = results[0]
    print(f"\nbest: {best_m:.4f} +/-{best_s:.4f}  {best_cfg}")
